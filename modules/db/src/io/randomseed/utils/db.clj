...
(defn invalidate+!
  ...
  ([f]
   (when-some [cache (cache-id+ f)] (reset! cache {}))
   (invalidate! f))
  ([f key-params]
   (when-some [cache (cache-id+ f)]
     (let [kp (if (vector? key-params) key-params (vec key-params))
           kp (if (> (count kp) 8) (conj (subvec kp 0 8) (seq (subvec kp 8))) kp)]
       (when (contains? @cache kp) (swap! cache dissoc kp))))
...
(defn make-getter-coll
  ...
  ([table id-col cols]
   (let [id-col (keyword id-col)
         ...
         q      (u/str-spc "SELECT" (join-col-names cols)
                           "FROM"   (or table "?")
...
(defn make-deleter
  ...
  ([table id-col]
   (let [id-col (keyword id-col)
         table  (to-snake-simple table)
         q      (u/str-spc "DELETE FROM" (or table "?")
                           "WHERE" (to-snake-simple id-col) "= ?")]
     (if table
       (fn db-deleter
         ([db _ id]
...
(defn for-replace
  ...
  (let [entity-fn (:table-fn opts identity)
    ...
    (into [(str "REPLACE INTO " (entity-fn (name table))
                " (" params ")"
                " VALUES (" places ")"
                (when-let [suffix (:suffix opts)] (str " " suffix)))]
          (vals key-map))))
...
(defn make-setting-getter
  ...
  (let [table           (to-snake-simple table)
        ...
        getter-query    (u/str-spc "SELECT value FROM" table
                                   "WHERE" entity-column "= ? AND id = ?")
        getter-subquery (u/str-spc "SELECT id, value FROM" table
    ...
    (fn get-setting
      ...
      ([db entity-id setting-id]
       (when-some [entity-id (id-to-db entity-id)]
         (when-some [setting-id (u/some-str setting-id)]
           (if db
           ...
           (when-some [setting-id (u/some-str setting-id)]
             (if-not db
               ...
               (let [setting-ids (cons setting-id (map id-to-db setting-ids))
                     query       (cons (u/str-spc getter-subquery (braced-join-? setting-ids))
...
(defn make-setting-setter
  ...
  [table entity-column]
  (let [table         (u/some-keyword-simple table)
        entity-column (u/some-keyword-simple entity-column)]
    (fn put-setting
             ...
             (if-not db
               ...
               (let [r (replace-multi!
                        ...
                        (->> pairs (cons value) (cons setting-id)
                             ...
                             (map (u/juxt-seq (constantly entity-id)
                                              (comp id-to-db first)
...
(defn make-setting-deleter
  ...
  (let [table-str         (to-snake-simple table)
        table             (u/some-keyword-simple table-str)
        entity-column-str (to-snake-simple entity-column)
        entity-column     (u/some-keyword-simple entity-column-str)
        ret-subquery      "RETURNING id"
        deleter-subquery  (u/str-spc "DELETE FROM" table-str
         ...
         (when-some [entity-id (id-to-db entity-id)]
           (when-some [setting-id (u/some-str setting-id)]
             (if-not db
               (log/err "Cannot delete settings in" table "for" entity-id
                        "because database connection is not set")
               (let [setting-ids (mapv u/some-str (cons setting-id setting-ids))
                     valsqls     (str "(" (str/join "," (repeat (count setting-ids) "?")) ")")
                     query       (cons (u/str-spc deleter-subquery valsqls) (cons entity-id setting-ids))]
  ...
  ([cache getter db entity-id setting-id & setting-ids]
   (if-let [setting-ids (prep-names setting-ids)]
     (let [entity-id    (id-as-str entity-id)
           ...
           found        (map-keys (comp keyword name) (dissoc looked-up-fq ::not-found))
  ...
  (reduce-kv
   (fn [acc id cfg]
     (let [cache-atom (var/deref id)
           cache-atom (if (u/atom? cache-atom) cache-atom (var/make id (atom nil)))
           cfg        (if (map? cfg) cfg {})]
