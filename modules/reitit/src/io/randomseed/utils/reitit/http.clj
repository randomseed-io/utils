...
(extend-protocol Routable
  ...
  (^Boolean router? [_] false)
  (^Router  router  [_] nil)

  Router

  (^Boolean router? [_] true)
  (^Router  router  [r] r)
  ...
  (router [x]
    (throw
     (ex-info "Cannot obtain router from object"
              {:type  ::r/unsupported
               :class (some-> x class .getName)
               :str   (try (str x) (catch Throwable _ "<unprintable>"))}))))

(extend-protocol Matchable

  Match

  (^Boolean match? [_] true)
  (^Match match
   (^Match [m]     m)
   (^Match [m _]   m)
   (^Match [m _ _] m))
  ...
  (match [x]
    (throw
     (ex-info "Cannot obtain match from object"
              {:type  ::r/unsupported
               :class (some-> x class .getName)
               :str   (try (str x) (catch Throwable _ "<unprintable>"))}))))
